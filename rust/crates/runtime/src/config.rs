...
pub const TERNLANG_CLI_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
pub enum McpServerConfig {
    ...
    TernlangAiProxy(McpTernlangAiProxyServerConfig),
}
...
impl ConfigLoader {
    ...
    pub fn default_for(cwd: impl Into<PathBuf>) -> Self {
        let cwd = cwd.into();
        let config_home = std::env::var_os("TERNLANG_CONFIG_HOME")
            .map(PathBuf::from)
            .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".ternlang")))
            .unwrap_or_else(|| PathBuf::from(".ternlang"));
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".ternlang.json"),
            |parent| parent.join(".ternlang.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".ternlang.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".ternlang").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".ternlang").join("settings.local.json"),
            },
        ]
    }
...
impl McpServerConfig {
    ...
    pub fn transport(&self) -> McpTransport {
        match self {
            ...
            Self::TernlangAiProxy(_) => McpTransport::TernlangAiProxy,
        }
    }
}
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".ternlang.json");
...
fn parse_mcp_server_config(
    ...
    match server_type {
        ...
        "ternlangai-proxy" => Ok(McpServerConfig::TernlangAiProxy(
            McpTernlangAiProxyServerConfig {
                url: expect_string(object, "url", context)?.to_string(),
...
mod tests {
    use super::{
        ...
        TERNLANG_CLI_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".ternlang");
    ...
    #[test]
    fn loads_and_merges_ternlang_cli_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".ternlang");
        fs::create_dir_all(cwd.join(".ternlang")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".ternlang.json"),
        ...
        fs::write(
            cwd.join(".ternlang.json"),
            r#"{"model":"project-compat","env":{"B":"2"}}"#,
        )
        .expect("write project compat config");
        fs::write(
            cwd.join(".ternlang").join("settings.json"),
            r#"{"env":{"C":"3"},"hooks":{"PostToolUse":["project"]},"mcpServers":{"project":{"command":"uvx","args":["project"]}}}"#,
        )
        .expect("write project settings");
        fs::write(
            cwd.join(".ternlang").join("settings.local.json"),
        ...
        assert_eq!(TERNLANG_CLI_SETTINGS_SCHEMA_NAME, "SettingsSchema");
...
fn parse_optional_providers(root: &JsonValue) -> Result<BTreeMap<String, ProviderConfig>, ConfigError> {
    let Some(object) = root.as_object() else {
    ...
    for (name, value) in providers {
        let provider = expect_object(value, &format!("merged settings.providers.{name}"))?;
        result.insert(
            name.clone(),
            ProviderConfig {
                api_key: optional_string(provider, "api_key", "merged settings.providers")?
                    .map(str::to_string),
                model: optional_string(provider, "model", "merged settings.providers")?
                    .map(str::to_string),
            },
        );
    }
...
fn parse_optional_default_provider(root: &JsonValue) -> Option<String> {
    root.as_object()
        .and_then(|object| object.get("default_provider"))
        .and_then(JsonValue::as_str)
        .map(ToOwned::to_owned)
}
