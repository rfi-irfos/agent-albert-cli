...
pub const FRONTIER_MODEL_NAME: &str = "Albert";
...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("ALBERT.md"),
            dir.join("ALBERT.local.md"),
            dir.join(".ternlang").join("ALBERT.md"),
            dir.join(".ternlang").join("instructions.md"),
        ] {
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "Ternlang instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# Ternlang instructions".to_string()];
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".ternlang")).expect("nested ternlang dir");
        fs::write(root.join("ALBERT.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("ALBERT.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".ternlang")).expect("apps ternlang dir");
        fs::write(root.join("apps").join("ALBERT.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".ternlang").join("instructions.md"),
            "apps dot ternlang instructions",
        )
        .expect("write apps dot ternlang instructions");
        fs::write(nested.join(".ternlang").join("ALBERT.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".ternlang").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("ALBERT.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("ALBERT.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.ternlang/ALBERT.md")),
            "ALBERT.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("ALBERT.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? ALBERT.md"));
    ...
    #[test]
    fn load_system_prompt_reads_ternlang_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".ternlang")).expect("ternlang dir");
        fs::write(root.join("ALBERT.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".ternlang").join("settings.json"),
        ...
        std::env::set_var("TERNLANG_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_ternlang_home {
            std::env::set_var("TERNLANG_CONFIG_HOME", value);
        } else {
    ...
    #[test]
    fn renders_ternlang_cli_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".ternlang")).expect("ternlang dir");
        fs::write(root.join("ALBERT.md"), "Project rules").expect("write ALBERT.md");
        fs::write(
            root.join(".ternlang").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        ...
        assert!(prompt.contains("# Ternlang instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    #[test]
    fn discovers_dot_ternlang_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".ternlang")).expect("nested ternlang dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".ternlang/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/ALBERT.md"),
            content: "Project rules".to_string(),
        }]);
        assert!(rendered.contains("# Ternlang instructions"));
